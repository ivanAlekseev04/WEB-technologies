...
$limit = isset($_GET['limit']) ? (int)$_GET['limit'] : 1; // Limit set to 1 for displaying one card per page
$offset = ($page - 1) * $limit;

$sql = $connection->prepare("SELECT * FROM cards JOIN users ON cards.sender_id=users.id WHERE receiver_id = :user_id LIMIT :limit OFFSET :offset");
...
$totalCardsQuery = $connection->prepare("SELECT COUNT(*) as total FROM cards WHERE receiver_id = :user_id");
